...
$metrics = Join-Path $repoRoot "results\fgvc_seed$Seed\q_${QMode}_$Steps.csv"
$checkpoint = Join-Path $repoRoot "checkpoints\fgvc_q_${QMode}_seed$Seed.pt"
$qOutput = Join-Path $repoRoot "gradient_gate_data\q_${QMode}_seed$Seed"
...
try {
    ...
    $arguments = @(
        ...
        "--method", "q", "--dataset", "fgvc", "--data_root", $DataRoot,
    ...
    & $python @arguments
    if ($LASTEXITCODE -ne 0) { throw "q $QMode seed $Seed failed with exit code $LASTEXITCODE" }
}
finally {
